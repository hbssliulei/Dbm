...
function loadIncBackupPic(url,game){
    $.ajax({
...
plotOptions :{
		...
		events: {
         ...
         var defautdate = day.getFullYear()+"-"+((day.getMonth()+1)<10?"0":"")+(day.getMonth()+1)+"-"+((day.getDate()-1)<10?"0":"")+(day.getDate()-1);
         ...
         if (enddate=="")
         {
			...
			$.ajax({
				url: '/api/getIncBackup/?game='+game+'&startdate='+startdate+'&enddate='+enddate,
				mimeType: 'textml; charset=utf-8', // ! Need set mimeType only when run from local file
				dataType: "json",
				type: 'GET',
				success: function(json){			   	
